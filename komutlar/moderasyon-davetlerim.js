...
exports.run = async (client, message, args, tools) => {
  ...
  if (!message.member.roles.has(veri)) {
    ...
    .setColor("BLUE")
      .setDescription(
        `${message.guild.roles.get(veri).name} Rolü İçin Son ${-sayı2 -
          -veri12} Davet!`
      )
      .setAuthor(`Slowex | Davet Sistemi`, message.author.avatarURL)
    .setFooter(`${message.author.tag} Tarafından İstendi`, message.author.avatarURL)
  ...
  if (message.member.roles.has(veri)) {
    if (!veri2) {
      const Pendra = new Discord.MessageEmbed()
        .addField(`Davetlerin Sahibi`, `<@` + kişi.id + `>`, true)
        .addField(`Total Davet:`, sayı2, true)
    .setColor("BLUE")
  .setAuthor(`Slowex | Davet Sistemi`, message.author.avatarURL)
  ...
  .setTimestamp()   
    ...
    .setColor("BLUE")
        .setDescription(
          `${message.guild.roles.get(veri2).name} Rolü İçin Son ${-sayı2 -
            -veri21} Davet!`
        )
        .setAuthor(`Slowex | Davet Sistemi`, message.author.avatarURL)
    .setFooter(`${message.author.tag} Tarafından İstendi`, message.author.avatarURL)
